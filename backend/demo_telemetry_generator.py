...
class GT3TelemetrySimulator:
    ...
    def _generate_coaching(self, tire_temps, brake_temps, throttle, brake, 
        ...
        if avg_tire_temp > 105:
            messages.append("⚠️ TIRE OVERHEAT: Reduce aggression, tires above 105°C")
        elif avg_tire_temp < 70:
            messages.append("ℹ️ Tires cold: Push harder to build temperature")
        
        # Brake temperature
        avg_brake_temp = sum(brake_temps) / len(brake_temps)
        if avg_brake_temp > 800:
            messages.append("🔥 BRAKE OVERHEAT: Reduce brake pressure, risk of fade")
        elif avg_brake_temp > 700:
            messages.append("⚠️ High brake temps: Consider cooling lap")
        
        # Throttle technique
        if throttle > 0.1 and brake > 0.1:
            messages.append("⚠️ TRAIL BRAKING: Overlapping brake/throttle detected")
        ...
        if distance < 0.02:
            if corner_name == "Eau Rouge":
                if throttle < 0.9:
                    messages.append(f"💡 {corner_name}: You can take this flat out!")
            elif corner_name == "Pouhon":
                messages.append(f"🎯 {corner_name}: Focus on smooth steering input")
        
        # Lap time comparison
        if self.best_lap_time < 999 and lap_time > self.best_lap_time + 2:
            messages.append(f"📊 Currently {lap_time - self.best_lap_time:.2f}s off best lap")
